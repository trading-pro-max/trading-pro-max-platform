# Founder Media Command

Founder Media Command defines how the private Founder Command App will review Media Office and AI Video Studio work without connecting accounts or publishing externally.

## Queues

- content draft queue
- AI video script queue
- campaign calendar
- channel readiness queue
- Guardian review queue
- Legal review queue
- Founder approval queue
- blocked/archived queue

## Content Lifecycle

| State | Meaning |
| --- | --- |
| draft | content exists but is not reviewed |
| guardian_review | Guardian checks abuse, manipulation, scam, and safety risk |
| legal_review | Legal Counsel checks claims and regulated wording |
| ready_for_founder | reviewed item is ready for Founder decision |
| approved | Founder approved the reviewed item |
| scheduled | future state after real publishing system exists |
| published | future state only after real external publishing exists |
| blocked | item cannot proceed |
| archived | retained for record/history |

## Future Channels

- X / Twitter
- Instagram
- TikTok
- YouTube
- Facebook
- LinkedIn
- Telegram
- Discord
- Reddit
- Blog / Newsroom

## Current Truth

- no social accounts connected
- no tokens configured
- no external posting
- no fake follower metrics
- no fake campaign performance
- no media automation shipped

## Blocked Content

- guaranteed profit
- win-rate claims
- risk-free wording
- sure signals
- fake live trading
- fake broker/feed/billing claims
- fake public launch
- fake Islamic/Sharia certification
- misleading urgency
- gambling-style marketing
- financial advice
- legal advice
- copied competitor content
## Deep Foundation Media / AI Video Command

Media Command and AI Video Studio Command are owner-only readiness surfaces.

Current truth:

- content draft readiness only
- AI video script readiness only
- campaign readiness planned/review-only
- social account registry not connected
- no social tokens
- no external publishing
- no media upload automation
- no fake followers
- no fake views
- no fake campaign metrics
- Legal review required
- Guardian review required
- Founder approval required

`/api/founder/media/readiness` reports this readiness truth only and cannot publish, connect accounts, upload media, schedule posts, or claim public reach.
## Economy / Growth Media Command Update

Founder Media Command sees Autonomous Media Office and AI Video Studio readiness as draft/review-only.

It must show:

- content draft readiness
- AI video script readiness
- campaign readiness
- social account registry readiness
- no connected accounts
- no social tokens
- no publishing
- no followers/views/ads metrics
- Legal, Guardian, and Founder review requirements
